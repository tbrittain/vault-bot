...
<a href="highscores.html" id="active">Highscores</a>
<hr>
...
<p>Some highscores for different aspects of the dynamic playlist. `r plot_title`</p>
<h2 id="genres"><a href="#genres">Genres</a></h2>
<p>Top Spotify-assigned genres currently in the dynamic playlist. Artists may belong to more than one genre (and some none at all if they are really underground). On average, each artist belongs to `r avg_genres` genres.
For more info on Spotify genres, check out <a href="http://everynoise.com" target=_blank>EveryNoise</a>, or click the link directly to the genre under the GenreLink column.</p>
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
...
<p>Top 25 artists whose songs are currently in the dynamic playlist.</p>
```{r, include=TRUE, echo=FALSE, message=FALSE, warning=FALSE}
